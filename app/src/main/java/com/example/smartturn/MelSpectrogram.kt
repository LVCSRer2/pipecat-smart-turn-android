...
class MelSpectrogram(context: Context) {
    ...
    init {
        ...
        Log.d("SmartTurnDemo", "MelSpectrogram initialized")
    }
    ...
    fun extractKotlin(audio: FloatArray, nFrames: Int = 800): Pair<FloatArray, Long> {
        ...
        // Pad or truncate to the required number of samples
        val requiredSamples = nFrames * hopLength + (windowSize - hopLength) // approx
        // Model expects exactly 128000 for some reason in native? No, it expects [1, 80, 800]
        // But the 8s window is 128000.
        val targetSize = 128000
        val input = FloatArray(targetSize)
        if (audio.size < targetSize) {
            System.arraycopy(audio, 0, input, targetSize - audio.size, audio.size)
        } else {
            System.arraycopy(audio, audio.size - targetSize, input, 0, targetSize)
        }
